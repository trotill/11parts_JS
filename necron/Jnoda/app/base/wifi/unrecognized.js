...
function WPASupp_DHCP(obj) {
	...
	if (obj.dhcp_en === 'true')
		serid[obj.devid]['dhcp'] = ex.Service(
			'udhcpc',
			`-f -i ${obj.IF} -s ${c.UDHCPC_CONFIG} -x hostname:${glob.websrv.cfg.hostname}`,
			'restart'
		);
			...
			wpa_emi.once('wpa_supl', (wpa_conf) => {
				...
				if (obj['Mode'] === 'client_in_br')
					serid[obj.devid]['wpas'] = ex.Service(
						...
						`-i${iface} -D${obj.Driver} -c${wpa_conf} -b br0`,
						'restart'
					);
				else
					serid[obj.devid]['wpas'] = ex.Service(
						`${sw_pth}.wpa_supplicant`,
						`-i${iface} -D${obj.Driver} -c${wpa_conf}`,
						'restart'
					);
			});
		}
